...
from trackers.chtracker.chtracker import CHTracker
    ...
    def evaluate_chtracker(
        ...
        for cur_iter, (imgs, _, info_imgs, ids, raw_image) in enumerate(    # [hgx0411] add raw_image for FastReID
            ...
            with torch.no_grad():
                ...
                if frame_id == 1:
                    tracker = CHTracker(args, det_thresh=self.args.track_thresh, iou_threshold=self.args.iou_thresh,
                ...
                ckt_file = "Detections/dancetrack/{}/{}_detetcion.pkl".format(video_name, img_base_name)
                if os.path.exists(ckt_file):
                    ...
                    os.makedirs("Detections/dancetrack/{}".format(video_name), exist_ok=True)
            ...
            # run tracking
            if self.args.with_fastreid == True:
                online_targets = tracker.update(outputs[0], info_imgs, self.img_size, id_feature=id_feature, warp_matrix=warp_matrix)
            else:
                online_targets = tracker.update(outputs[0], info_imgs, self.img_size, warp_matrix=warp_matrix)
            online_tlwhs = []
            online_ids = []
